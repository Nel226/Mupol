...
class AyantDroitsImport implements ToModel, WithHeadingRow
{
    ...
    public function model(array $row)
    {
        try {
            ...
            $adherantExists = Adherant::where('no_matricule', $row['matricule_adherant'])->exists();
            if (!$adherantExists) {
            ...
            // Générer le code
            $data = Adherant::pluck('no_matricule')->toArray();
            ...
            $ayantDroit->adherant_id = Adherant::where('no_matricule', $row['matricule_adherant'])->value('id');
