        ...
        <div class="flex-1 p-6">
            ...
            <div class="p-6 mx-auto mt-4 bg-white rounded-lg shadow-lg ">
                <div id="tab-recapitulatif-content" class="mt-2 tab-content">
                    <h1 class="pb-3 text-xl font-bold text-center underline ">Suivi des radiographies pour {{ $currentYear }}</h1>
                   
                    <div class="flex items-center justify-between py-4 text-sm">
                        <div class="flex gap-3 ">

                            <form method="GET" action="{{ route('suivi-radio') }}"  onsubmit="showSpinner()">
                    ...
                    <div class="grid grid-cols-1 gap-4 mb-6 md:grid-cols-2">
                        ...
                        <!-- Graphique Coût total des radio -->
                       <div>
                           <h3>Coût total des radios (E)</h3>
                           <canvas id="prestationsCostChart"></canvas>
                       </div>  

                        <!-- Graphique Nombre de radio -->
                        <div>
                            <span>Nombre de radiographies (B)</span>
                            <canvas id="radiosChart"></canvas>
                        </div>
                    ...
                    <script>
                        ...
                        const radiosData = @json(array_values($data['Nombre de radios (B)']));
                        const prestationsCostData = @json(array_values($data['Coût total des radios (E)']));
                        const avgRadioCostData = @json(array_values($data['Coût moyen mensuel d’une radio (F)']));


                        const ctx1 = document.getElementById('prestationsCostChart').getContext('2d');
                        ...
                        const ctx2 = document.getElementById('radiosChart').getContext('2d');
                        ...
                        new Chart(ctx1, {
                            ...
                            data: {
                                labels: months,
                                datasets: [{
                                    label: 'Coût total des radiographies',
                                ...
                                {
                                    label: 'Coût moyen mensuel d’une radio (F)',
                                    data: avgRadioCostData.slice(0, 12), 
                        ...
                        // Graphique Nombre de radios
                        new Chart(ctx2, {
                            type: 'bar',
                            data: {
                                labels: months,
                                datasets: [{
                                    label: 'Nombre de radios',
                                    data: radiosData.slice(0, 12), // Exclure les totaux et moyennes
