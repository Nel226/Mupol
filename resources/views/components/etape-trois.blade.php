...
<div>
    ...
    <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div id="photoUpload" class="col-span-1">
            <label class="block text-gray-700 text-sm font-bold mb-1" for="photo">                    
                Photo
                <small class=" text-primary1">(Taille max : 2Mo)</small>
            </label>
    ...
    <div class="space-y-4">
        <div class="overflow-x-auto">
            ...
            <span id="nombreAyantsDroitsError" class="text-red-500 text-sm" style="display:none;"></span>
        </div>
        ...
        <script>
                ...
                for (let i = 0; i < nombreAyantsDroits; i++) {
                    ...
                    div.innerHTML = `
                        ...
                        <div id="cnib-field-${i}" class="mt-4" style="display:none;">
                            <label class="block text-gray-700 text-sm font-bold mb-1">CNIB                     
                                <small class=" text-primary1">(en PDF, Taille max : 2Mo)</small>
                            </label>
                            <input type="file" name="ayantsDroits[${i}][cnib]" class="w-full py-2" accept=".pdf">
                        </div>
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-1">Photo de l'ayant droit
                                    <small class=" text-primary1">(Taille max : 2Mo)</small>
                                </label>
                                <input type="file" name="ayantsDroits[${i}][photo]" class="w-full py-2" accept="image/jpg, image/jpeg, image/png">
                                <span class="error-message text-red-500 text-xs mt-1"></span>
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-1">Extrait d'acte de naissance
                                    <small class=" text-primary1">(en PDF, Taille max : 2Mo)</small>
            ...
            function handleRelationChange(index) {
                ...
                if (relationSelect.value === 'Conjoint(e)') {
        ...
        <div>
            <label class="block text-gray-700 text-sm font-bold mb-1" for="signature">Signature</label>
    
            <!-- Charger une image de la signature -->
            <div id="uploadSignature" class=" flex flex-col p-4 border rounded-md bg-white h-full shadow-sm">
                <h4 class="text-center text-sm font-semibold mb-4">Charger une image claire de votre signature
                    <small class=" text-primary1">(Taille max : 2Mo)</small>
                </h4>
                <div class="border-dashed border-2 border-gray-300 rounded-md p-6 flex-grow flex justify-center items-center">
                    <div class="text-center text-xs">
                        ...
                        <label for="signature" class="cursor-pointer text-blue-500 hover:underline">cliquez pour choisir une image</label>
                        <input type="file" id="signature"  class="hidden" accept="image/*" name="signature" >
                    </div>
                </div>
            </div>
            <span id="signature_error" class="text-red-500 text-xs"></span>

            <script>
                const signatureImageInput = document.getElementById('signature');
